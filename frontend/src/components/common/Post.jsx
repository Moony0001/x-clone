	...
	const {mutate: deletePost, isPending: isDeleting} = useMutation({
		mutationFn: async () => {
			try {
				...
				if(!res.ok){
					throw new Error(data.error || "Post couldn't be deleted");
				}
				return data;
			} catch (error) {
	...
	const {mutate: likePost, isPending: isLiking} = useMutation({
		mutationFn: async () => {
			try {
				...
				const data = await res.json();
				if(!res.ok){
					throw new Error(data.error || "Post couldn't be liked");
				}
				return data;
			} catch (error) {
		...
		onSuccess: (updatedLikes) => {
			...
			//this is not the best UX because upon liking one post, all the posts will be refetched
			// queryClient.invalidateQueries({queryKey: ["posts"]});
			//instead we update the cache directly of that post.
